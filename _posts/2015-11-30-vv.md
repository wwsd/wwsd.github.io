...
Which starts with ‘shee’ or ‘#’ and ‘band’ or ‘!’.  I hereby declare that no one is to call the ‘!’ an ‘exclamation point’ from here on out we wil call it bang! And hash is now know as ‘shee’.  Anyhow, after your shebang you call an absolute path to your interpreter, which in this case is ‘bash’ and bash lives at `/bin/bash` on most systems, if not try `which bash`, and the which command will tell you where bash lives on your system
